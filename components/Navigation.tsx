...
export default function Navigation() {
  ...
  return (
    <>
      <nav className="sticky top-0 z-50 w-full glass border-b border-border/40 px-4 md:px-8">
        <div className="max-w-7xl mx-auto flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-3">
            ...
            <button 
              className="md:hidden p-2 text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors"
      ...
      {/* Mobile Menu - Outside of Sticky Nav to avoid transformation/transparency inheritance */}
      <AnimatePresence>
        {isOpen && (
          <div className="fixed inset-0 z-[100] md:hidden">
            ...
            <motion.div 
              ...
              className="absolute top-0 right-0 bottom-0 w-[280px] bg-white dark:bg-slate-900 shadow-[0_0_50px_rgba(0,0,0,0.3)] p-6 flex flex-col z-50"
            >
              <div className="flex items-center justify-between mb-8 border-b border-border/50 pb-4">
                <span className="font-black text-xl tracking-tight text-slate-900 dark:text-white">
                  Building<span className="text-primary">Calc</span>
                </span>
                <button 
                  onClick={() => setIsOpen(false)}
                  ...
                  return (
                    <Link
                      ...
                      className={`
                        ...
                        ${isActive 
                          ? 'bg-primary/10 text-primary shadow-sm' 
                          : 'text-slate-600 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-white'
                        }
                      `}
                    >
                      <Icon size={24} />
              ...
              <div className="pt-6 border-t border-border/50 mt-auto">
                <Link 
                  href="/projects/new"
                  onClick={() => setIsOpen(false)}
                  className="flex items-center justify-center gap-2 w-full bg-primary text-white py-4 rounded-2xl font-black shadow-xl shadow-primary/20 hover:shadow-primary/40 active:scale-95 transition-all text-lg"
                >
                  <Calculator size={20} />
                  Proiect Nou
                </Link>
